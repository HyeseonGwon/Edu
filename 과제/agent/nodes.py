...
nodes.py
========
LangGraph 워크플로우를 구성하는 4개의 핵심 노드를 정의합니다.

    ┌──────────────────┐   정보 부족 → 되물음(END)
    │ 1. State Manager │────────────────────────────► (사용자에게 질문 후 대기)
    └───────┬──────────┘
            │ 정보 충족
            ▼
    ┌──────────────────┐
    │ 2. Candidate Gen │  웹 검색으로 1차 후보 ~10곳 도출
    └───────┬──────────┘
            ▼
    ┌──────────────────┐
    │ 3. Deep Validator│  ★가장 중요★ 계단/메뉴 2조건 검증 → 3~5곳만 통과
    └───────┬──────────┘
            ▼
    ┌──────────────────┐
    │ 4. Map Builder   │  통과 후보를 Folium 지도로 시각화
    └──────────────────┘

공통 설계 원칙 (발표 포인트)
--------------------------
- 모든 노드는 15.1 실습대로 `state: FamilyTripState` 를 받아 `dict` 를 반환합니다.
- 모든 노드는 try/except 로 감싸, 검색/파싱 실패에도 그래프가 멈추지 않고
  "정보를 찾기 어렵습니다" 식으로 부드럽게 넘어갑니다. (실시간 시연 안정성)
...
SEARCH_RADIUS_KM = 10.0    # 지도 '검색 영역' 원의 반경(km). 이 반경 안에서 확실히 찾은 곳만 핀 표시
...
# ══════════════════════════════════════════════════════════════════════════
# 노드 1. State Manager (상태 관리자)
#  - 대화에서 3단계(누구와/무엇을/어디로)를 파악해 구조화(JSON)합니다.
#  - 부족하면 되물을 질문을 만들고, 다 모이면 검색 파이프라인으로 넘어가게 합니다.
# ══════════════════════════════════════════════════════════════════════════
class CollectResult(BaseModel):
    ...
    companions: str = Field(default="", description="누구와 함께 가는지")
    place_type: str = Field(default="", description="무엇을 찾는지 (식당/숙소/둘다)")
    region: str = Field(default="", description="어디로 = 검색 지역")
...
STATE_MANAGER_SYSTEM = (
    ...
    "규칙:\n"
    "- 지금까지의 대화 '전체'를 근거로 현재까지 파악된 값을 채우세요. 이미 말한 정보는 유지하세요.\n"
...
def state_manager_node(state: FamilyTripState) -> dict:
    """[노드 1] 대화를 분석해 요구조건을 구조화하고, 다음 행동(질문 vs 검색)을 결정한다.
    ...
    if not info_complete:
        ...
        if not companions:
            ...
            question = "어느 지역으로 가시나요? (예: 서울 송파구)"
        else:
...
def search_candidates(
    ...
    Returns:
        새 후보 Place 리스트(중복/기존 이름 제외). 검색 실패 시 빈 리스트.
    """
    ...
    system = (
        ...
        f"사용자는 '{region}' 지역의 '{place_type}'를 찾고 있습니다.\n\n"
        "[반드시 지켜야 할 규칙]\n"
        ...
        f"2. 반드시 '{region}'에 실제로 위치한 곳만 포함하세요. 스니펫에 '{region}' 또는 그 안의\n"
        "   동/도로명 주소가 함께 나오는 곳을 우선하고, 그 근거가 있으면 in_region=True 로 두세요.\n"
        ...
        f"5. address 에는 스니펫에서 확인되는 '{region}' 포함 주소/위치를 적으세요. 없으면 빈 값." + exclude_hint
    )
    try:
        extracted: CandidateList = candidate_extractor.invoke(
            [
                ...
                HumanMessage(
                    content=(
                        f"[검색 스니펫]\n{snippet_text}\n\n"
                        f"위에서 '{region}'에 실제로 있는 '{place_type}'의 상호명만 추출하세요. "
                        "메뉴·음식 이름과 다른 지역 가게는 제외하세요."
                    )
                ),
            ]
        )
    ...
    # 코드 레벨에서도 중복/기존 이름/지역-무관 후보를 한 번 더 걸러 냅니다(안전장치).
    ...
    for item in extracted.places:
        ...
        if not name or name.lower() in seen_lower:
            continue
        # 지역 근거가 없다고 LLM 이 스스로 표시(in_region=False)한 곳은 제외 (지역 정확도↑)
        if not item.in_region:
...
def validate_place(place: Place, req: TripRequirements, label: str = "") -> bool:
    ...
    try:
        evidence_text = (
            ...
            f"[계단 관련 리뷰]\n{stair_evidence or '(근거 없음)'}\n\n"
            f"[메뉴 관련 리뷰]\n{menu_evidence or '(근거 없음)'}"
        )
...
def build_map_html(req: TripRequirements | None, finalists: list[Place]) -> str:
    """검색 영역(원)과 '위치가 확실한 후보'의 핀만 그린 Folium 지도 HTML 을 만든다.

    핀 정확도 정책 (요청 사항 반영)
        - 지도에는 먼저 '검색 영역'을 원(Circle)으로 그립니다. (지역 중심 반경 SEARCH_RADIUS_KM)
    ...
    try:
        ...
        folium.Circle(
            ...
            radius=SEARCH_RADIUS_KM * 1000,  # m 단위
            color="#3186cc",
            weight=2,
            fill=True,
            fill_opacity=0.08,
            popup=folium.Popup(f"검색 영역: '{region}' 중심 반경 약 {SEARCH_RADIUS_KM:.0f}km", max_width=220),
        ).add_to(fmap)
        # 지역 중심 표식(작게)
        folium.Marker(
            location=list(center),
        ...
        for p in finalists:
            ...
            if not p.geo_checked:
                p.geo_checked = True
                coords = locate_place(p.name, p.address, region, center, SEARCH_RADIUS_KM)
